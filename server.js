...
app.post('/api/rooms', (req, res) => {
  ...
  const template = floorTemplates.find((t) => t.id === floorTemplateId) || floorTemplates[0];
  const room = {
    id: uuidv4(),
    code,
    name,
...
// Socket.IO realtime and signaling
const participantSockets = new Map(); // participantId -> socket
...
io.on('connection', (socket) => {
  ...
  socket.on('update-position', ({ roomCode, seatId, x, y }) => {
    ...
    socket.to(roomCode).emit('participant-moved', { participantId, seatId, x, y });
  });

  socket.on('signal', ({ targetId, data }) => {
    const targetSocket = participantSockets.get(targetId);
